...
mod tests {
    ...
    #[test]
    fn jumps_backward_with_zero_flag() {
        let mut vm = VirtualMachine::default();
        vm.registers.write(Register::PC, 0x0100);
        vm.registers.write(Register::F, flags::Z_FLAG as u16);

        vm.execute(&Opcode::Jzr { offset: -0x02 });

        assert_eq!(vm.registers.read(Register::PC), 0x00FE);
    }
}
