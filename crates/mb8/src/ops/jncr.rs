...
impl VirtualMachine {
    pub fn jncr(&mut self, offset: i8) {
...
mod tests {
    ...
    #[test]
    fn jumps_when_carry_flag_clear() {
        let mut vm = VirtualMachine::default();
        vm.program_counter = 0x0100;
        vm.registers.write(Register::F, 0);
        vm.execute(&Opcode::Jncr { offset: 0x20 });
        assert_eq!(vm.program_counter, 0x0120);
    }

    #[test]
    fn does_not_jump_when_carry_flag_set() {
        let mut vm = VirtualMachine::default();
        vm.program_counter = 0x0100;
        vm.registers.write(Register::F, flags::C_FLAG);
        vm.execute(&Opcode::Jncr { offset: 0x20 });
        assert_eq!(vm.program_counter, 0x0100);
    }

    #[test]
    fn jumps_backward_when_carry_flag_clear() {
        let mut vm = VirtualMachine::default();
        vm.program_counter = 0x0100;
        vm.registers.write(Register::F, 0);
        vm.execute(&Opcode::Jncr { offset: -0x20 });
